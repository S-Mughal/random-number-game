...
for (int i=0; i<x.length; i++) { for (int j=0; j<a.length; j++) { x[j]=a[j]; } x[totalGamesPlayed-1] = newGuess; } return x; } int totalUsers; //stores total users int minN; 
  //stores min range int maxN; 
  //stores max range int interval; 
  //stores interval int nGames=1; 
  //stores number of games played boolean numberError = false; String userString; ArrayList usersRandNum = new ArrayList<>();
...
{
for (int i=0; i<pName.size(); i++) { int existingIndex = findExistingPlayer(pName.get(i)); if (existingIndex == -1) { 
